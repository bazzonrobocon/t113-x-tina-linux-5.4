...
int snd_sunxi_jack_register(struct snd_soc_card *card)
{
	...
	/* Notice: the char name "Jack" can't be moved or changed, cause it was used to detect and
	 * distinguish the input event at dragonboard testcase */
	ret = snd_soc_card_jack_new(sunxi_jack.card, "Headphones Jack",
				    SND_JACK_HEADSET
				    | SND_JACK_HEADPHONE
				    | SND_JACK_BTN_0
				    | SND_JACK_BTN_1
