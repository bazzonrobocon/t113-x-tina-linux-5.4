...
#ifdef SUNXI_CODEC_SWAP_ENABLE
/* sunxi codec swap func */
static const char * const sunxi_codec_swap_function[] = {
				"Off", "On"};

static const struct soc_enum sunxi_codec_swap_func_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(sunxi_codec_swap_function),
			sunxi_codec_swap_function),
};
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	reg_val = snd_soc_component_read32(component, SUNXI_DAC_DG);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	switch (ucontrol->value.integer.value[0]) {
	case	0:
		snd_soc_component_update_bits(component, SUNXI_DAC_DG,
...
#endif
#ifdef SUNXI_CODEC_SWAP_ENABLE
	SOC_ENUM_EXT("DAC Swap", sunxi_codec_swap_func_enum[0],
				sunxi_codec_get_dacswap_mode,
				sunxi_codec_set_dacswap_mode),

	SOC_ENUM_EXT("ADC Swap", sunxi_codec_swap_func_enum[0],
...
#if IS_ENABLED(CONFIG_SND_SUNXI_JACK_CODEC)
/* jack work -> codec */
...
static int sunxi_jack_codec_init(void *data)
{
	...
	regmap_update_bits(regmap, SUNXI_MICBIAS_REG, 0x3 << SELDETADCDB, 0x3 << SELDETADCDB);
...
static void sunxi_jack_codec_det_irq_work(void *data, enum snd_jack_types *jack_type)
{
	...
	case JACK_IRQ_OUT:
		...
		if (jack_codec_priv->hpmic_adc_swap > 0) {
			regmap_update_bits(regmap, SUNXI_ADC_DG,
					0x1 << AD_SWP, 0x0 << AD_SWP);
		}
...
#elif IS_ENABLED(CONFIG_SND_SUNXI_JACK_EXTCON)
/* jack work -> extcon */
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	case 2:
		...
		break;
	default:
		SND_LOG_ERR(HLOG, "only support mono or stereo mode.\n");
		return -EINVAL;
	}
			...
			int clk_id, unsigned int freq, int dir)
{
	...
	case	0:
		...
		if (clk_set_parent(sunxi_codec->dacclk, sunxi_codec->pllcomdiv5)) {
			SND_LOG_ERR(HLOG, "set parent of dacclk to pllcomdiv5 failed\n");
			return -EINVAL;
		}
	...
	case	1:
		...
		if (clk_set_parent(sunxi_codec->adcclk, sunxi_codec->pllcomdiv5)) {
			SND_LOG_ERR(HLOG, "set parent of adcclk to pllcomdiv5 failed\n");
			return -EINVAL;
		}
	...
	case	2:
		...
		if (clk_set_parent(sunxi_codec->dacclk, sunxi_codec->pllclk)) {
			SND_LOG_ERR(HLOG, "set parent of dacclk to pllclk failed\n");
			return -EINVAL;
		}
	...
	case	3:
		...
		if (clk_set_parent(sunxi_codec->adcclk, sunxi_codec->pllclk)) {
			SND_LOG_ERR(HLOG, "set parent of adcclk to pllclk failed\n");
			return -EINVAL;
		}
		...
		break;
	default:
		SND_LOG_ERR(HLOG, "Bad clk params input!\n");
		return -EINVAL;
	}
...
#if IS_ENABLED(CONFIG_SND_SUNXI_JACK_CODEC) || IS_ENABLED(CONFIG_SND_SUNXI_JACK_EXTCON)
	struct sunxi_codec_info *codec = snd_soc_component_get_drvdata(component);
#endif
...
#if IS_ENABLED(CONFIG_SND_SUNXI_JACK_CODEC)
	/* jack init -> codec */
	codec->jack_codec_priv.regmap = codec->regmap;
...
#elif IS_ENABLED(CONFIG_SND_SUNXI_JACK_EXTCON)
	/* jack init -> extcon */
	codec->jack_extcon_priv.regmap = codec->regmap;
...
static int sunxi_codec_resume(struct snd_soc_component *component)
{
	...
	if (sunxi_codec->vol_supply.avcc) {
		...
		if (ret)
			SND_LOG_ERR(HLOG, "resume avcc enable failed!\n");
	}

	if (sunxi_codec->vol_supply.cpvin) {
		ret = regulator_enable(sunxi_codec->vol_supply.cpvin);
		if (ret)
			SND_LOG_ERR(HLOG, "resume cpvin enable failed!\n");
	}

	if (clk_set_rate(sunxi_codec->pllcom, 451584000)) {
		SND_LOG_ERR(HLOG, "resume codec source set pllcom rate failed\n");
		return -EBUSY;

	}

	if (clk_set_rate(sunxi_codec->pllclk, 98304000)) {
		SND_LOG_ERR(HLOG, "resume codec source set pllclk rate failed\n");
		return -EBUSY;
	}

	if (reset_control_deassert(sunxi_codec->codec_clk_rst)) {
		SND_LOG_ERR(HLOG, "resume deassert the codec reset failed\n");
		return -EBUSY;
	}

	if (clk_prepare_enable(sunxi_codec->codec_clk_bus)) {
		SND_LOG_ERR(HLOG, "enable codec bus clk failed, resume exit\n");
		return -EBUSY;
	}

	if (clk_prepare_enable(sunxi_codec->pllclk)) {
		SND_LOG_ERR(HLOG, "enable pllclk failed, resume exit\n");
		return -EBUSY;
	}

	if (clk_prepare_enable(sunxi_codec->pllcom)) {
		SND_LOG_ERR(HLOG, "enable pllcom failed, resume exit\n");
		return -EBUSY;
	}

	if (clk_prepare_enable(sunxi_codec->pllcomdiv5)) {
		SND_LOG_ERR(HLOG, "enable pllcomdiv5 failed, resume exit\n");
		return -EBUSY;
	}

	if (clk_prepare_enable(sunxi_codec->dacclk)) {
		SND_LOG_ERR(HLOG, "enable dacclk failed, resume exit\n");
		return -EBUSY;
	}

	if (clk_prepare_enable(sunxi_codec->adcclk)) {
		SND_LOG_ERR(HLOG, "enable  adcclk failed, resume exit\n");
		clk_disable_unprepare(sunxi_codec->adcclk);
		return -EBUSY;
	}
				...
				const char *buf, size_t count)
{
	...
	SND_LOG_INFO(HLOG, "ret:%d, reg_offset:%d, reg_val:0x%x\n",
			ret, input_reg_offset, input_reg_val);

	if (!(rw_flag == 1 || rw_flag == 0)) {
		SND_LOG_ERR(HLOG, "not rw_flag\n");
		ret = count;
		goto out;
	}

	if (input_reg_offset > SUNXI_BIAS_REG) {
		SND_LOG_ERR(HLOG, "the reg offset[0x%03x] > SUNXI_BIAS_REG[0x%03x]\n",
				...
				struct sunxi_codec_info *sunxi_codec)
{
	...
	if (IS_ERR(sunxi_codec->vol_supply.avcc)) {
		SND_LOG_ERR(HLOG, "get audio avcc failed\n");
		goto err_regulator;
	} else {
		ret = regulator_set_voltage(sunxi_codec->vol_supply.avcc,
							1800000, 1800000);
		if (ret) {
			SND_LOG_ERR(HLOG, "audiocodec avcc set vol failed\n");
	...
	if (IS_ERR(sunxi_codec->vol_supply.cpvin)) {
		SND_LOG_ERR(HLOG, "get cpvin failed\n");
		goto err_regulator_avcc;
	} else {
		ret = regulator_set_voltage(sunxi_codec->vol_supply.cpvin,
							1800000, 1800000);
		if (ret) {
			SND_LOG_ERR(HLOG, "cpvin set vol failed\n");
			goto err_regulator_cpvin;
		}

		ret = regulator_enable(sunxi_codec->vol_supply.cpvin);
		if (ret) {
			SND_LOG_ERR(HLOG, "cpvin enable failed!\n");
			goto err_regulator_cpvin;
		}
	}
				...
				struct sunxi_codec_info *sunxi_codec)
{
	...
	if (reset_control_deassert(sunxi_codec->codec_clk_rst)) {
		SND_LOG_ERR(HLOG, "deassert the codec reset failed\n");
		goto err_devm_kfree;
	}

	if (clk_set_parent(sunxi_codec->pllcomdiv5, sunxi_codec->pllcom)) {
		SND_LOG_ERR(HLOG, "set parent of pllcomdiv5 to pllcom failed\n");
		goto err_devm_kfree;
	}
	...
	if (clk_set_rate(sunxi_codec->pllcom, 451584000)) {
		SND_LOG_ERR(HLOG, "codec source set pllcom rate failed\n");
		goto err_devm_kfree;
	}

	if (clk_set_rate(sunxi_codec->pllclk, 98304000)) {
		SND_LOG_ERR(HLOG, "codec source set pllclk rate failed\n");
		goto err_devm_kfree;
	}

	if (clk_prepare_enable(sunxi_codec->codec_clk_bus)) {
		SND_LOG_ERR(HLOG, "codec clk bus enable failed\n");
	}

	if (clk_prepare_enable(sunxi_codec->pllclk)) {
		SND_LOG_ERR(HLOG, "pllclk enable failed\n");
		goto err_bus_kfree;
	}

	if (clk_prepare_enable(sunxi_codec->pllcom)) {
		SND_LOG_ERR(HLOG, "pllcom enable failed\n");
		goto err_pllclk_kfree;
	}

	if (clk_prepare_enable(sunxi_codec->pllcomdiv5)) {
		SND_LOG_ERR(HLOG, "pllcomdiv5 enable failed\n");
	...
	if (clk_prepare_enable(sunxi_codec->adcclk)) {
		SND_LOG_ERR(HLOG, "moduleclk enable failed\n");
				    ...
				    struct sunxi_codec_info *sunxi_codec)
{
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "headphonegain volume get failed, use default vol\n");
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "mic1gain get failed, use default vol\n");
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "mic2gain get failed, use default vol\n");
...
#if IS_ENABLED(CONFIG_SND_SUNXI_JACK_CODEC)
	/* jack param -> codec */
	ret = of_property_read_u32(np, "hpmic_adc_swap", &temp_val);
	if (ret < 0) {
		sunxi_codec->jack_codec_priv.hpmic_adc_swap = 0;
	} else {
		sunxi_codec->jack_codec_priv.hpmic_adc_swap = temp_val;
	}
	pr_err("hpmic_adc_swap: %d\n", sunxi_codec->jack_codec_priv.hpmic_adc_swap);
	ret = of_property_read_u32(np, "jack-det-level", &temp_val);
	if (ret < 0) {
		sunxi_codec->jack_codec_priv.det_level = 0;
	} else {
		if (temp_val > 0)
			sunxi_codec->jack_codec_priv.det_level = 1;
		else
			sunxi_codec->jack_codec_priv.det_level = 0;
	}
	ret = of_property_read_u32(np, "jack-det-threshold", &temp_val);
	if (ret < 0 || temp_val > 0x1f) {
		sunxi_codec->jack_codec_priv.det_threshold = 8; /* AW1855 default 8 */
	} else {
		sunxi_codec->jack_codec_priv.det_threshold = temp_val;
	}
	ret = of_property_read_u32(np, "jack-det-debouce-time", &temp_val);
	if (ret < 0 || temp_val > (125 * (0xf + 1))) {
		sunxi_codec->jack_codec_priv.det_debouce_time = 250; /* AW1855 default 250ms */
	} else {
		sunxi_codec->jack_codec_priv.det_debouce_time = temp_val;
	}
#elif IS_ENABLED(CONFIG_SND_SUNXI_JACK_EXTCON)
	/* jack param -> extcon */
	ret = of_property_read_u32(np, "hpmic_adc_swap", &temp_val);
	if (ret < 0) {
		sunxi_codec->jack_extcon_priv.hpmic_adc_swap = 0;
	} else {
		sunxi_codec->jack_extcon_priv.hpmic_adc_swap = temp_val;
	}
	pr_err("hpmic_adc_swap: %d\n", sunxi_codec->jack_extcon_priv.hpmic_adc_swap);
	ret = of_property_read_u32(np, "jack-det-level", &temp_val);
	if (ret < 0) {
		sunxi_codec->jack_extcon_priv.det_level = 0;
	} else {
		if (temp_val > 0)
			sunxi_codec->jack_extcon_priv.det_level = 1;
		else
			sunxi_codec->jack_extcon_priv.det_level = 0;
	}
	ret = of_property_read_u32(np, "jack-det-threshold", &temp_val);
	if (ret < 0 || temp_val > 0x1f) {
		sunxi_codec->jack_extcon_priv.det_threshold = 8; /* AW1855 default 8 */
	} else {
		sunxi_codec->jack_extcon_priv.det_threshold = temp_val;
	}
	ret = of_property_read_u32(np, "jack-det-debouce-time", &temp_val);
	if (ret < 0 || temp_val > (125 * (0xf + 1))) {
		sunxi_codec->jack_extcon_priv.det_debouce_time = 250; /* AW1855 default 250ms */
	} else {
		sunxi_codec->jack_extcon_priv.det_debouce_time = temp_val;
	}
#endif
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "pa_msleep get failed, use default vol\n");
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "pa_level get failed, use default vol\n");
		sunxi_codec->spk_config.pa_level = 1;
	} else {
		sunxi_codec->spk_config.pa_level = temp_val;
	}

	SND_LOG_DEBUG(HLOG, "digital_vol:%d, lineout_vol:%d, "
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "adcdrc_cfg configs missing or invalid.\n");
	} else {
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "adchpf_cfg configs missing or invalid.\n");
	} else {
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "dacdrc_cfg configs missing or invalid.\n");
	} else {
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "dachpf_cfg configs missing or invalid.\n");
	} else {
		sunxi_codec->hw_config.dachpf_cfg = temp_val;
	}

	SND_LOG_DEBUG(HLOG, "adcdrc_cfg:%d, adchpf_cfg:%d, "
	...
	if (ret >= 0) {
		...
		if (!gpio_is_valid(sunxi_codec->spk_config.spk_gpio)) {
			SND_LOG_ERR(HLOG, "gpio-spk is invalid\n");
			return -EINVAL;
		} else {
			ret = devm_gpio_request(&pdev->dev,
				sunxi_codec->spk_config.spk_gpio, "SPK");
			if (ret) {
				SND_LOG_ERR(HLOG, "failed to request gpio-spk\n");
				return -EBUSY;
			}
		}
	} else {
		sunxi_codec->spk_config.used = 0;
		SND_LOG_ERR(HLOG, "gpio-spk no exist\n");
	}
...
static int sunxi_internal_codec_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(np)) {
		SND_LOG_ERR(HLOG, "pdev->dev.of_node is err.\n");
		ret = -EFAULT;
		goto err_node_put;
	}
	...
	sunxi_codec->pdev = pdev;
	sunxi_codec->dev = &pdev->dev;

	if (sunxi_codec_regulator_init(pdev, sunxi_codec) != 0) {
		SND_LOG_ERR(HLOG, "Failed to init sunxi audio regulator\n");
		ret = -EFAULT;
		goto err_devm_kfree;
	}

	if (sunxi_codec_clk_init(np, pdev, sunxi_codec) != 0) {
		SND_LOG_ERR(HLOG, "Failed to init sunxi audio clk\n");
		ret = -EFAULT;
		goto err_devm_kfree;
	}
	...
	if (ret != 0) {
		SND_LOG_WARN(HLOG, "Failed to parse sunxi audio params\n");
		if (ret == -EPROBE_DEFER) {
			SND_LOG_WARN(HLOG, "probe get some params failed and retry probe!\n");
			goto err_devm_kfree;
		}
	}
	...
	if (ret) {
		SND_LOG_ERR(HLOG, "Failed to get sunxi resource\n");
		return -EINVAL;
	...
	if (!memregion) {
		SND_LOG_ERR(HLOG, "sunxi memory region already claimed\n");
		ret = -EBUSY;
		goto err_moduleclk_disable;
	}
	...
	if (!sunxi_codec->digital_base) {
		SND_LOG_ERR(HLOG, "sunxi digital_base ioremap failed\n");
		ret = -EBUSY;
		goto err_moduleclk_disable;
	}
	...
	if (ret < 0) {
		SND_LOG_ERR(HLOG, "register codec failed\n");
		goto err_moduleclk_disable;
	}

	ret  = sysfs_create_group(&pdev->dev.kobj, &audio_debug_attr_group);
	if (ret) {
		SND_LOG_ERR(HLOG, "failed to create attr group\n");
		goto err_moduleclk_disable;
	}

	SND_LOG_INFO(HLOG, "audiocodec probe finished.\n");
...
static int  __exit sunxi_internal_codec_remove(struct platform_device *pdev)
{
	...
	SND_LOG_INFO(HLOG, "audiocodec remove finished.\n");
...
static int __init sunxi_internal_codec_driver_init(void)
{
	return platform_driver_register(&sunxi_internal_codec_driver);
}
late_initcall(sunxi_internal_codec_driver_init);

static void __exit sunxi_internal_codec_driver_exit(void)
{
	platform_driver_unregister(&sunxi_internal_codec_driver);
}
module_exit(sunxi_internal_codec_driver_exit);
