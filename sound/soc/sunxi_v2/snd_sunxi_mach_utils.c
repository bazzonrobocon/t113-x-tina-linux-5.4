...
{
	if (!dais) {
		SND_LOG_ERR(HLOG, "simple dais null\n");
		return -1;
	}
...
int asoc_simple_parse_misc(struct device_node *node, char *prefix, struct asoc_simple_priv *priv)
{
	...
	if (ret) {
		priv->jack_support = false;
	} else {
		if (val == 1) {
			priv->jack_support = true;
			SND_LOG_DEBUG(HLOG, "jack is supported\n");
		} else {
			priv->jack_support = false;
			SND_LOG_DEBUG(HLOG, "jack is unsupported\n");
		}
	}
