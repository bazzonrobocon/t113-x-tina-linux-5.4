...
struct sunxi_jack {
	...
	/* create jack dev only when support is true */
	bool support;
...
extern void sunxi_jack_typec_mode_set(struct sunxi_jack_typec_cfg *jack_typec_cfg,
...
extern int snd_sunxi_jack_register(struct snd_soc_card *card);
extern void snd_sunxi_jack_unregister(struct snd_soc_card *card);
#elif IS_ENABLED(CONFIG_SND_SOC_SUNXI_JACK_GPIO)
extern int snd_sunxi_jack_gpio_init(void *jack_data);
extern void snd_sunxi_jack_gpio_exit(void *jack_data);
extern int snd_sunxi_jack_register(struct snd_soc_card *card);
extern void snd_sunxi_jack_unregister(struct snd_soc_card *card);
#else
...
{
	(void)jack_data;
	pr_debug("[sound %4d][PCM %s] JACK API is disabled\n", __LINE__, __func__);
	return 0;
}
...
static inline int snd_sunxi_jack_gpio_init(void *jack_data)
{
	(void)jack_data;
	pr_debug("[sound %4d][PCM %s] JACK API is disabled\n", __LINE__, __func__);
	return 0;
...
static inline void snd_sunxi_jack_gpio_exit(void *jack_data)
{
	(void)jack_data;
	pr_debug("[sound %4d][PCM %s] JACK API is disabled\n", __LINE__, __func__);
	return;
