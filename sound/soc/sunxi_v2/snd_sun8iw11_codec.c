...
static void snd_sunxi_dts_params_init(struct platform_device *pdev, struct sunxi_codec_dts *dts)
{
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "adc gain get failed\n");
		dts->adc_gain = 3;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "mic1 gain get failed\n");
		dts->mic1_gain = 4;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "mic2 gain get failed\n");
		dts->mic2_gain = 4;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "mic1 to omix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "mic2 to omix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "fmin to omix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "linein to omix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "lineinl to romix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "lineinr to lomix gain get failed\n");
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "phoneout gain get failed\n");
		dts->phoneout_gain = 3;
	} else {
	...
	SND_LOG_DEBUG(HLOG, "dac_vol:%u, adc_gain:%u, mic1_gain:%u, mic2_gain:%u\n",
		      dts->dac_vol, dts->adc_gain, dts->mic1_gain, dts->mic2_gain);
	SND_LOG_DEBUG(HLOG, "mic1_to_omix_gain:%u, mic2_to_omix_gain:%u, fmin_to_omix_gain:%u\n",
		      dts->mic1_to_omix_gain, dts->mic2_to_omix_gain, dts->fmin_to_omix_gain);
	SND_LOG_DEBUG(HLOG, "linein_to_omix_gain:%u, lineinl_to_romix_gain:%u, "
		      "lineinr_to_lomix_gain:%u\n", dts->linein_to_omix_gain,
		      dts->lineinl_to_romix_gain, dts->lineinr_to_lomix_gain);
	SND_LOG_DEBUG(HLOG, "phoneout_gain:%u, hpout_gain:%u\n",
