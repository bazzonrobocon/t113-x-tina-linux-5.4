/*
 * sound\soc\sunxi\snd_sunxi_owa.c
...
{
	struct sunxi_owa *owa = snd_soc_dai_get_drvdata(dai);
	struct sunxi_owa_clk *clk = &owa->clk;
...
static int sunxi_owa_set_data_fmt(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct sunxi_owa *owa = snd_soc_component_get_drvdata(component);
...
static int sunxi_owa_get_data_fmt(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct sunxi_owa *owa = snd_soc_component_get_drvdata(component);
...
static const struct snd_kcontrol_new sunxi_owa_controls[] = {
	SOC_ENUM_EXT("audio data format", data_fmt_enum,
		     sunxi_owa_get_data_fmt, sunxi_owa_set_data_fmt),
};
...
static int snd_sunxi_pin_init(struct platform_device *pdev, struct sunxi_owa_pinctl *pin)
{
	...
	if (ret < 0) {
		SND_LOG_ERR(HLOG, "owa set pinctrl default state fail\n");
		ret = -EBUSY;
...
static void snd_sunxi_dump_version(void *priv, char *buf, size_t *count)
{
	...
	if (!owa) {
		SND_LOG_ERR(HLOG, "priv to owa failed\n");
		return;
	}
	if (owa->pdev)
...
static int sunxi_owa_dev_probe(struct platform_device *pdev)
{
	...
	if (!owa) {
		SND_LOG_ERR(HLOG, "can't allocate sunxi owa memory\n");
		ret = -ENOMEM;
	...
	SND_LOG_DEBUG(HLOG, "register owa platform success\n");
...
static int sunxi_owa_dev_remove(struct platform_device *pdev)
{
	...
	SND_LOG_DEBUG(HLOG, "unregister owa platform success\n");
