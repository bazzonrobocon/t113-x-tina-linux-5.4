...
int sunxi_hdmi_extcon_init(struct sunxi_codec *codec)
{
	...
	if (!codec) {
		SND_LOG_ERR(HLOG, "codec is invaild\n");
		return -1;
	}
	...
	ret = extcon_register_notifier(hdmi_extcon->extdev, EXTCON_DISP_HDMI,
				       &hdmi_extcon->hdmi_nb);
	if (ret < 0) {
		SND_LOG_ERR(HLOG, "register hdmi notifier failed\n");
		return -1;
	}
	return 0;
}
...
static int sunxi_codec_dev_probe(struct platform_device *pdev)
{
	...
	if (!codec) {
		SND_LOG_ERR(HLOG, "can't allocate sunxi codec hdmi memory\n");
		ret = -ENOMEM;
	...
	if (ret) {
		SND_LOG_ERR(HLOG, "hdmi extcon init failed\n");
	}

	/* alsa component register */
	ret = snd_soc_register_component(dev, &sunxi_codec_component_dev, &sunxi_codec_dai, 1);
	if (ret) {
		SND_LOG_ERR(HLOG, "hdmi-codec component register failed\n");
		ret = -ENOMEM;
		goto err_devm_kzalloc;
	}

	SND_LOG_ERR(HLOG, "register codec-hdmi success\n");
...
static int sunxi_codec_dev_remove(struct platform_device *pdev)
{
	...
	SND_LOG_ERR(HLOG, "unregister codec-hdmi success\n");
