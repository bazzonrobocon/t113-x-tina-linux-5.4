/*
 * sound\soc\sunxi\snd_sun8iw18_codec.c
				     ...
				     struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		case 2:
			...
			break;
		default:
			SND_LOG_WARN(HLOG, "cannot support the channels:%u.\n",
...
static int snd_sunxi_clk_enable(struct sunxi_codec_clk *clk)
{
	...
	if (clk_prepare_enable(clk->clk_bus_audio)) {
		SND_LOG_ERR(HLOG, "clk_bus_audio enable failed\n");
	...
	if (clk_prepare_enable(clk->clk_audio)) {
		SND_LOG_ERR(HLOG, "clk_audio enable failed\n");
...
static void snd_sunxi_dts_params_init(struct platform_device *pdev, struct sunxi_codec_dts *dts)
{
	...
	SND_LOG_DEBUG(HLOG, "adc-gain		-> %u\n", dts->adc_gain);
	SND_LOG_DEBUG(HLOG, "mic1-gain		-> %u\n", dts->mic1_gain);
	SND_LOG_DEBUG(HLOG, "mic2-gain		-> %u\n", dts->mic2_gain);
	SND_LOG_DEBUG(HLOG, "mic3-gain		-> %u\n", dts->mic2_gain);
	SND_LOG_DEBUG(HLOG, "dac-dig-vol	-> %u\n", dts->dac_dig_vol);
	SND_LOG_DEBUG(HLOG, "lineout-vol	-> %u\n", dts->lineout_vol);
	SND_LOG_DEBUG(HLOG, "adc-delay-time	-> %u\n", dts->adc_delay_time);
	SND_LOG_DEBUG(HLOG, "lineout-single     -> %s\n",
