...
static int snd_sunxi_hw_effect_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (IS_ERR_OR_NULL(sfx)) {
		SND_LOG_ERR(HLOG, "snd_sunxi_sfx is NULL\n");
		return -1;
	}

	if (vm_size > ((sfx->phys_addr_size / PAGE_SIZE + 1) * PAGE_SIZE)) {
		SND_LOG_ERR(HLOG, "mmap vm_size is too long\n");
		return -1;
	}
	...
	if (ret) {
		SND_LOG_ERR(HLOG, "io_remap_pfn failed\n");
		return -1;
	}
...
int snd_sunxi_hw_effect_init(struct snd_sunxi_sfx *sfx)
{
	...
	if (IS_ERR_OR_NULL(sfx->sfx_dev_name)
		...
		SND_LOG_ERR(HLOG, "snd_sunxi_sfx name member is NULL\n");
		return -1;
	}
	...
	SND_LOG_DEBUG(HLOG, "sfx major = %u, sfx minor = %u\n",
	...
	if (ret) {
		SND_LOG_ERR(HLOG, "cdev_add failed\n");
		goto err_cdev_add;
	}
	...
	if (IS_ERR_OR_NULL(sfx->sfx_device)) {
		SND_LOG_ERR(HLOG, "device_create failed\n");
