/*
 * sound\soc\sunxi\snd_sunxi_codec_hdmi.c
...
static void sunxi_codec_dai_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (ret)
		SND_LOG_ERR(HLOG, "edp_audio_disable failed\n");
}
...
static int sunxi_codec_dai_prepare(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	...
	if (ret) {
		SND_LOG_ERR(HLOG, "edp_audio_set_para failed\n");
		return ret;
	}

	ret = codec->edp_func.edp_audio_enable(0);
	if (ret) {
		SND_LOG_ERR(HLOG, "edp_audio_enable failed\n");
		return ret;
	}
...
static int sunxi_codec_component_probe(struct snd_soc_component *component)
{
	...
	if (ret) {
		SND_LOG_ERR(HLOG, "snd_edp_get_func failed\n");
		return -1;
	}

	if (!codec->edp_func.edp_audio_enable) {
		SND_LOG_ERR(HLOG, "edp_audio_enable func is null\n");
		return -1;
	}

	if (!codec->edp_func.edp_audio_disable) {
		SND_LOG_ERR(HLOG, "edp_audio_disable func is null\n");
		return -1;
	}

	if (!codec->edp_func.edp_audio_set_para) {
		SND_LOG_ERR(HLOG, "edp_audio_set_para func is null\n");
		return -1;
	}
...
static int sunxi_codec_dev_probe(struct platform_device *pdev)
{
	...
	if (!codec) {
		SND_LOG_ERR(HLOG, "can't allocate sunxi codec edp memory\n");
		ret = -ENOMEM;
	...
	if (ret) {
		SND_LOG_ERR(HLOG, "edp-codec component register failed\n");
		ret = -ENOMEM;
		goto err_devm_kzalloc;
	}

	SND_LOG_DEBUG(HLOG, "register codec-edp success\n");
...
static int sunxi_codec_dev_remove(struct platform_device *pdev)
{
	...
	SND_LOG_DEBUG(HLOG, "unregister codec-edp success\n");
