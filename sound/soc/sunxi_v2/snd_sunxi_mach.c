...
static int simple_parse_of(struct asoc_simple_priv *priv)
{
	...
	if (ret < 0) {
		SND_LOG_ERR(HLOG, "asoc_simple_parse_misc failed\n");
		return ret;
	}
...
static void snd_sunxi_dump_version(void *priv_orig, char *buf, size_t *count)
{
	...
	if (!priv) {
		SND_LOG_ERR(HLOG, "priv_orig to priv failed\n");
		return;
	}
...
static int snd_sunxi_dump_show(void *priv_orig, char *buf, size_t *count)
{
	...
	count_tmp += sprintf(buf + count_tmp, "MCLK FP     -> %s\n", mclk_fp?"On":"Off");
...
static int simple_soc_probe(struct snd_soc_card *card)
{
	...
	if (!priv) {
		SND_LOG_ERR(HLOG, "snd_soc_card priv invailed\n");
		return -1;
	}

	if (asoc_simple_is_i2sdai(priv->dais)) {
		dump = &priv->dump;
		snprintf(priv->module_name, 32, "%s-%s", "machaine", card->name);
