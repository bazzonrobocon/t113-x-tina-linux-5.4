/* sound\soc\sunxi\snd_sunxi_test_clk.c
...
static int snd_sunxi_pin_init(struct platform_device *pdev, struct sunxi_test_clk_pinctl *pin)
{
	...
	if (ret < 0) {
		SND_LOG_ERR(HLOG, "test set pinctrl default state fail\n");
		ret = -EBUSY;
...
static int sunxi_test_dev_probe(struct platform_device *pdev)
{
	...
	if (!sunxi_test) {
		SND_LOG_ERR(HLOG, "can't allocate sunxi_test memory\n");
		ret = -ENOMEM;
	...
	if (ret) {
		SND_LOG_ERR(HLOG, "set clk rate failed\n");
