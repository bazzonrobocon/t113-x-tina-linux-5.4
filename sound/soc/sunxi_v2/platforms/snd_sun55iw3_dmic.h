 ...
 *
 */
#ifndef __SND_SUN55IW3_DMIC_H
#define __SND_SUN55IW3_DMIC_H
...
static int snd_sunxi_clk_rate(struct sunxi_dmic_clk *clk, unsigned int freq_in,
...
static inline int snd_sunxi_clk_rate(struct sunxi_dmic_clk *clk, unsigned int freq_in,
				     unsigned int freq_out)
{
	SND_LOG_DEBUG(HLOG, "\n");
	...
	if (freq_in % 24576000 == 0) {
		...
		if (clk_set_parent(clk->clk_dmic, clk->clk_pll_audio0_4x)) {
			SND_LOG_ERR(HLOG, "set dmic parent clk failed\n");
			return -EINVAL;
		}
	...
	if (clk_set_rate(clk->clk_dmic, freq_out)) {
		SND_LOG_ERR(HLOG, "set clk_dmic rate failed, rate: %u\n", freq_out);
		return -EINVAL;
	}
