...
int sunxi_rx_sync_probe(rx_sync_domain_t domain)
{
	...
	if (domain < 0 || domain >= RX_SYNC_DOMAIN_CNT) {
		SND_LOG_ERR(HLOG, "unsupport domain id:%d\n", domain);
		return -EINVAL;
	}
...
EXPORT_SYMBOL(sunxi_rx_sync_register_cb);
...
EXPORT_SYMBOL(sunxi_rx_sync_unregister_cb);
