...
#include "snd_sunxi_jack_utils.h"
...
jack_report:
	...
	break;
	default:
		SND_LOG_DEBUG(HLOG, "jack setup status is invaild\n");
	break;
	}
			       ...
			       enum sunxi_jack_modes mode)
{
	...
	if (!modes_map || !jack_pins) {
		SND_LOG_ERR(HLOG, "modes map or jack pins is NULL\n");
		return;
	}

	if (mode >= SND_JACK_MODE_CNT || modes_map[mode].type == SND_JACK_MODE_NULL) {
		SND_LOG_WARN(HLOG, "missing mode value,mode:%d\n", mode);
		return;
	}
...
static int sunxi_jack_typec_init(struct sunxi_jack_extcon *jack_extcon)
{
	...
	if (ret < 0) {
		SND_LOG_WARN(HLOG, "jack-swpin-max get failed,stop init\n");
		return 0;
	} else {
	...
	if (!jack_typec_cfg->jack_pins) {
		SND_LOG_ERR(HLOG, "can't get pin_config memory\n");
		return -ENOMEM;
	}
	...
	if (!jack_typec_cfg->modes_map) {
		SND_LOG_ERR(HLOG, "can't get pin_mode_cfg memory\n");
		return -ENOMEM;
	}
	...
	for (i = 0; i < ARRAY_SIZE(of_mode_table); ++i) {
		map_value = devm_kcalloc(&pdev->dev, jack_typec_cfg->sw_pin_max, sizeof(uint32_t),
					 GFP_KERNEL);
		if (!map_value) {
			SND_LOG_ERR(HLOG, "can't get map value memory\n");
			return -ENOMEM;
		}
		...
		if (ret) {
			...
			SND_LOG_DEBUG(HLOG, "mode:%s get failed,will not set mode map\n",
...
int snd_sunxi_jack_extcon_init(void *jack_data)
{
	...
	if (!sunxi_jack.support) {
		SND_LOG_DEBUG(HLOG, "jack do not support\n");
		return 0;
	}
