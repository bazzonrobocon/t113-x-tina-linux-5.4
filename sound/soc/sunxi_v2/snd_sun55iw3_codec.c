/*
 * sound\soc\sunxi\snd_sun55iw3_codec.c
...
static void sunxi_jack_codec_det_irq_work(void *data, enum snd_jack_types *jack_type)
{
	...
	break;
	case JACK_IRQ_MIC:
		SND_LOG_DEBUG(HLOG, "jack button\n");
		...
		if (reg_val >= jack_codec_priv->key_det_vol[0][0] &&
			...
			*jack_type |= SND_JACK_BTN_3;
			...
			SND_LOG_DEBUG(HLOG, "unsupport jack button\n");
		}
	break;
	default:
...
static int sunxi_codec_component_probe(struct snd_soc_component *component)
{
	...
	mutex_init(&codec->mic_sta.mic_sta_mutex);
...
static void sunxi_codec_component_remove(struct snd_soc_component *component)
{
	...
	mutex_destroy(&codec->mic_sta.mic_sta_mutex);
...
static int snd_sunxi_clk_init(struct platform_device *pdev, struct sunxi_codec_clk *clk)
{
	...
	if (IS_ERR_OR_NULL(clk->clk_peri0_2x)) {
		SND_LOG_ERR(HLOG, "clk peri0 2x get failed\n");
	...
	if (IS_ERR_OR_NULL(clk->clk_dsp_src)) {
		SND_LOG_ERR(HLOG, "clk dsp src get failed\n");
...
static int snd_sunxi_rglt_init(struct platform_device *pdev, struct sunxi_codec_rglt *rglt)
{
	...
	if (rglt->vdd_external) {
		SND_LOG_DEBUG(HLOG, "use external vdd\n");
		...
		if (IS_ERR_OR_NULL(rglt->vdd)) {
			...
			if (ret < 0) {
				SND_LOG_ERR(HLOG, "set vdd voltage failed\n");
				ret = -EFAULT;
				goto err_rglt;
			}
			ret = regulator_enable(rglt->vdd);
			if (ret < 0) {
				SND_LOG_ERR(HLOG, "enable vdd failed\n");
				ret = -EFAULT;
				goto err_rglt;
			}
		}
	...
	if (rglt->cpvin_external) {
		SND_LOG_DEBUG(HLOG, "use external cpvin\n");
		...
		if (IS_ERR_OR_NULL(rglt->cpvin)) {
			...
			if (ret < 0) {
				SND_LOG_ERR(HLOG, "set cpvin voltage failed\n");
				ret = -EFAULT;
				goto err_rglt;
			}
			ret = regulator_enable(rglt->cpvin);
			if (ret < 0) {
				SND_LOG_ERR(HLOG, "enable cpvin failed\n");
				ret = -EFAULT;
				goto err_rglt;
			}
		}
	} else {
		SND_LOG_ERR(HLOG, "unsupport internal cpvin for headphone charge pump\n");
		ret = -EFAULT;
		goto err_rglt;
	}
...
static void snd_sunxi_dts_params_init(struct platform_device *pdev, struct sunxi_codec_dts *dts)
{
...
#if IS_ENABLED(CONFIG_SND_SOC_SUNXI_JACK_CODEC) || IS_ENABLED(CONFIG_SND_SOC_SUNXI_JACK_EXTCON)
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "dac volume get failed\n");
		dts->dac_vol = 0;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "dacl volume get failed\n");
		dts->dacl_vol = 160;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "dacr volume get failed\n");
		dts->dacr_vol = 160;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "adc1 volume get failed\n");
		dts->adc1_vol = 160;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "adc2 volume get failed\n");
		dts->adc2_vol = 160;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "adc3 volume get failed\n");
		dts->adc3_vol = 160;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "lineout gain get failed\n");
		dts->lineout_gain = 31;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "hpout gain get failed\n");
		dts->hpout_gain = 0;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "adc1 gain get failed\n");
		dts->adc1_gain = 31;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "adc2 gain get failed\n");
		dts->adc2_gain = 31;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "adc3 gain get failed\n");
		dts->adc3_gain = 31;
	} else {
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "jack-key-det-voltage-hook get failed\n");
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "jack-key-det-voltage-up get failed\n");
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "jack-key-det-voltage-down get failed\n");
	...
	if (ret < 0) {
		SND_LOG_DEBUG(HLOG, "jack-key-det-voltage-voice get failed\n");
	...
	SND_LOG_DEBUG(HLOG, "dac_vol:%u, dacl_vol:%u, dacr_vol:%u\n",
		      dts->dac_vol, dts->dacl_vol, dts->dacr_vol);
	SND_LOG_DEBUG(HLOG, "adc1_vol:%u, adc2_vol:%u, adc3_vol:%u\n",
		      dts->adc1_vol, dts->adc2_vol, dts->adc3_vol);
	SND_LOG_DEBUG(HLOG, "lineout_gain:%u, hpout_gain:%u\n",
		      dts->lineout_gain, dts->hpout_gain);
	SND_LOG_DEBUG(HLOG, "adc1_gain:%u, adc2_gain:%u, adc3_gain:%u\n",
...
static void snd_sunxi_dump_version(void *priv, char *buf, size_t *count)
{
	...
	if (!codec) {
		SND_LOG_ERR(HLOG, "priv to codec failed\n");
		return;
	}
	if (codec->pdev)
