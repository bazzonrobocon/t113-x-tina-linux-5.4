...
static __maybe_unused void hw_yymmdd_hhmmss_to_tm(const struct sunxi_rtc_data *data,
...
static __maybe_unused void tm_to_hw_yymmdd_hhmmss(const struct sunxi_rtc_data *data,
...
static int sunxi_rtc_reboot_flag_setup(struct sunxi_rtc_dev *chip)
{
	...
	if (err) {
		...
		boot_reasonbase = chip->base + chip->data->gpr_offset + gpr_bootcount * 0x4;
	}
