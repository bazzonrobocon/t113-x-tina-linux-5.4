...
#define PCIE_DMA_WR_CHN_CNT		0x5
#define PCIE_DMA_RD_CHN_CNT		0x4
...
#ifdef CONFIG_PCIE_SUNXI_DMA
struct dma_trx_obj *sunxi_pcie_dma_obj_probe(struct device *dev);
void sunxi_pcie_dma_obj_remove(struct dma_trx_obj *obj);
...
#else
static inline struct edma_trx_obj *sunxi_pcie_dma_obj_probe(struct device *dev)
{
	return NULL;
}

static inline void sunxi_pcie_dma_obj_remove(struct dma_trx_obj *obj)
{
	return NULL;
}
#endif
