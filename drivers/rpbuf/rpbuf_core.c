...
struct rpbuf_controller *rpbuf_get_controller_by_of_node(const struct device_node *np, int index)
{
	...
	dev = bus_find_device_by_of_node(&platform_bus_type, rpbuf_np);
	if (!dev) {
				...
				struct rpbuf_buffer *buffer)
{
	...
	if (wait_event_interruptible(buffer->wait, buffer->state != BUFFER_WAIT))
			return -ERESTARTSYS;
	return 0;
}
							    ...
							    void *content)
{
	...
	if (buffer->state != BUFFER_WAIT) {
		dev_warn(controller->dev, "invalid buffer(%d) state\n", cont->id);
		mutex_unlock(&link->controller_lock);
		ret = -EINVAL;
		goto err_out;
	}
			  ...
			  unsigned int offset, unsigned int data_len)
{
	...
	/*
	 * Remove restriction of the range of
	 * offset and data_len, to allow more
	 * flexible usage on rpbuf (e.g. use
	 * rpbuf as a ring buffer
	 */
#if 0
