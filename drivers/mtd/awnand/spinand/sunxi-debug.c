...
static ssize_t aw_spinand_show_uid(struct aw_spinand *spinand, char *buf);
...
static struct attribute attr_uid = {
	.name = "uid",
	.mode = S_IRUGO,
};
...
#define UID_LEN	32
	...
	struct aw_spinand_chip_request req = {0};
	u8 reg, sou_reg;
	unsigned char uid_buf[UID_LEN] = {0x00};
	unsigned int pagesize = 1 << PAGE_SHIFT;
	unsigned int rx_bit = chip->rx_bit;

	ret = ops->get_otp(chip, &sou_reg);
	if (ret)
		goto err;
	/* Enable OTP_EN */
	reg = (sou_reg | CFG_OTP_ENABLE) & ~CFG_QUAD_ENABLE;
	ret = ops->set_otp(chip, reg);
	if (ret)
		goto err;
	/* Check OTP whether enable */
	ret = ops->get_otp(chip, &reg);
	if (ret)
		goto err;
	if (!(reg & CFG_OTP_ENABLE))
		goto err;
	...
	if ((0xC8 == chip->info->phy_info->NandID[0]) && (0xb1 == chip->info->phy_info->NandID[1])) {
		req.page = 6;
	}
	...
	/* Disable OTP_EN */
	ret = ops->set_otp(chip, sou_reg);
	if (ret)
		goto err;

	ret = 0;
	ret += snprintf(buf + ret, pagesize - ret, "UID:");
	for (i = 0; i < UID_LEN; i++) {
		ret += snprintf(buf + ret, pagesize - ret, " %u", uid_buf[i]);
	}
	ret += snprintf(buf + ret, pagesize - ret, "\n");
