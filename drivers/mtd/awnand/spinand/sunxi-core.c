...
download_err:
	if (end_tmp > end) {
		pr_err("no enough space to download uboot from blk %u to block %u\n", start, end_tmp);
		return -ENOSPC;
	} else if (blks_written < 0) {
		pr_err("failed to download uboot from blk %u to blk %u \n", start, end_tmp);
		return blks_written;
	} else if (blks_written < blks_per_uboot) {
		pr_err("written %u blks but wanted %u blks\n", blks_written, blks_per_uboot);
		return -EINVAL;
	}
		...
		unsigned int len, void *buf)
{
	...
	struct aw_spinand_chip *chip = spinand_to_chip(spinand);
	struct aw_spinand_info *info = chip->info;
	unsigned int phy_blk_size, blks_per_uboot;
	unsigned int start, end, download_flag;
	int ret;
