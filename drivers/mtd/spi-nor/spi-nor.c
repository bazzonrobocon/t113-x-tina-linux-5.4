...
#define SPI_NOR_STACK_DIE	BIT(16) /* flash supports to use multi dies */
...
/**
 * sunxi_select_die()	-compatity of two_die flash, choose which die to use.
 * @nor:	pointer to a 'struct spi_nor'
 * @die_num:	specified die to use
 *		0:die0  1:die1
 *
...
static int sunxi_select_die(struct spi_nor *nor, u8 select_die)
{
	...
	if (select_die != nor->active_stack_die) {
		...
		if (nor->spimem) {
			struct spi_mem_op op =
				SPI_MEM_OP(SPI_MEM_OP_CMD(SPINOR_OP_SDS, 1),
				SPI_MEM_OP_ADDR(1, select_die, 1),
				SPI_MEM_OP_NO_DUMMY,
				SPI_MEM_OP_NO_DATA);
			...
			ret = nor->write_reg(nor, SPINOR_OP_SDS, &select_die, 1);
		}
...
erase_die:
	...
	if ((nor->info->flags & SPI_NOR_STACK_DIE) && total_stack_die) {
...
static const struct flash_info spi_nor_ids[] = {
	...
	{ "w25m512jv", INFO(0xef7119, 0, 64 * 1024, 1024,
			SECT_4K | SPI_NOR_QUAD_READ | SPI_NOR_DUAL_READ | SPI_NOR_STACK_DIE | SPI_NOR_SKIP_SFDP) },
	...
	{ "py25q128", INFO(0x852018, 0, 4 * 1024, 4096, SECT_4K|SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ) },
	...
	{ "xt25f256b", INFO(0x0b4019, 0x0, 64 * 1024, 512, SECT_4K | SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ
						| SPI_NOR_4B_OPCODES)},
...
static int security_regiser_erase(struct mtd_info *mtd, loff_t addr)
{
	...
	write_enable(nor);
	ret = spi_nor_write_data(nor, addr, 0, NULL);
	ret = spi_nor_wait_till_ready(nor);
					...
					loff_t addr, loff_t len, u_char *buf)
{
	...
	while (len) {
		if (nor->bouncebuf && len > nor->bouncebuf_size)
			size = nor->bouncebuf_size;
		else
			size = len;

		ret = spi_nor_read_data(nor, addr, size, buf);
		if (ret == 0) {
			/* We shouldn't see 0-length reads */
			ret = -EIO;
			goto read_err;
					...
					loff_t addr, loff_t len, u_char *buf)
{
	...
	while (len) {
		...
		write_enable(nor);
		ret = spi_nor_write_data(nor, addr, size, buf);
		spi_nor_wait_till_ready(nor);
		if (ret == 0) {
			/* We shouldn't see 0-length reads */
			ret = -EIO;
