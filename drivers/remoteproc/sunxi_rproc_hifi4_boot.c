...
static int devm_sunxi_rproc_hifi4_resource_get(struct sunxi_rproc_priv *rproc_priv, struct platform_device *pdev)
{
	...
	hifi4_cfg->msg_rst = devm_reset_control_get(dev, "msg-rst");
	if (IS_ERR_OR_NULL(hifi4_cfg->msg_rst)) {
		dev_err(dev, "no find dbg in dts\n");
		return -ENXIO;
	}
...
static int sunxi_rproc_hifi4_start(struct sunxi_rproc_priv *rproc_priv)
{
	...
	/* reset msgbox */
	ret = reset_control_reset(hifi4_cfg->msg_rst);
	if (ret) {
		dev_err(dev, "rproc reset err\n");
		return ret;
	}
	...
	/* set msgbox to deassert  */
	ret = reset_control_deassert(hifi4_cfg->msg_rst);
	if (ret) {
		dev_err(dev, "set msgbox to deassert err\n");
		return ret;
	}
...
static int sunxi_rproc_hifi4_stop(struct sunxi_rproc_priv *rproc_priv)
{
	...
	clk_disable(hifi4_cfg->cfg_clk);

	clk_disable(hifi4_cfg->mod_clk);
