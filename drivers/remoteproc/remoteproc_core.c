...
static int rproc_start(struct rproc *rproc, const struct firmware *fw)
{
	...
	if (power == 1) {
		...
		dev_dbg(dev, "Booting remote process success\n");
	} else {
	...
	if ((rproc->state == RPROC_EARLY_BOOT) && (rproc->auto_boot)) {
		dev_info(dev, "Fastboot remote process\n");
		ret = rproc->ops->attach(rproc);
		if (ret) {
			dev_err(dev, "can't attach rproc %s: %d\n", rproc->name, ret);
...
int rproc_boot(struct rproc *rproc)
{
	...
	if ((rproc->state == RPROC_EARLY_BOOT) && (rproc->auto_boot)) {
		if (rproc->ops->request_firmware) {
			ret = rproc->ops->request_firmware(rproc, &firmware_p);
			if (ret < 0) {
				dev_err(dev, "request_firmware of driver ops failed: %d\n", ret);
				goto downref_rproc;
			}
		} else {
			dev_err(dev, "the driver did not implement request_firmware api\n");
			goto downref_rproc;
		}
	} else {
		ret = request_firmware(&firmware_p, rproc->firmware, dev);
...
int rproc_add(struct rproc *rproc)
{
	...
	if (rproc->auto_boot) {
		if (rproc->ops->trigger_auto_boot)
			ret = rproc->ops->trigger_auto_boot(rproc);
		else
