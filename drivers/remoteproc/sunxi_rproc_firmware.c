...
static void decompress_timeout_handler(struct timer_list *t)
{
	struct fw_mem_info *info = from_timer(info, t, timer);
	Elf32_Ehdr *ehdr = info->addr;

#ifdef CONFIG_ARM64
	__dma_flush_area((void *)ehdr, sizeof(*ehdr));
#else
			   ...
			   const char *name, struct device *dev)
{
	...
	if ((ehdr->e_machine & 0xff00) == 0xff00) {
		...
		timer_setup(&info->timer, decompress_timeout_handler, 0);
