				...
				unsigned long addr)
{
	...
	/* ensure that the address is successfully written */
	for (i = 0; i < 20; i++) {
		if ((addr >> ADDR_BIT_R_SHIFT) != vin_reg_readl(csic_dma_base[sel] + CSIC_DMA_F0_BUFA_REG_OFF + buf * 4))
			vin_reg_clr_set(csic_dma_base[sel] + CSIC_DMA_F0_BUFA_REG_OFF + buf * 4,
					F0_BUFA_MASK, addr >> ADDR_BIT_R_SHIFT);
		else
			return;
	}
	vin_err("CSI dma buffer address write failed!\n");
#endif
#else
