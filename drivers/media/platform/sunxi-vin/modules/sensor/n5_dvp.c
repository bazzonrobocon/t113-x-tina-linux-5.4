...
void n5_set_chn_1080p_25(struct v4l2_subdev *sd, unsigned char chn)
{
	data_type val;
...
void n5_set_chn_720h_pal(struct v4l2_subdev *sd, unsigned char chn)
{
	data_type val;
	sensor_dbg("%s chn=%d\n", __func__, chn);
	sensor_write(sd, 0xff, 0x00);
	sensor_write(sd, 0x08 + chn, 0xdd);
	sensor_write(sd, 0x34 + chn, 0x00);
	sensor_write(sd, 0x81 + chn, 0x70);
	sensor_write(sd, 0x85 + chn, 0x00);

	sensor_read(sd, 0x54, &val);
	sensor_write(sd, 0x54, val & (~(0x10 << chn)));
	sensor_write(sd, 0x18 + chn, 0x08);
	sensor_write(sd, 0x58 + chn, 0x80);
	sensor_write(sd, 0x5c + chn, 0xbe);
	sensor_write(sd, 0x64 + chn, 0xa0);
	sensor_write(sd, 0x89 + chn, 0x10);
	sensor_write(sd, chn + 0x8e, 0x2e);
	sensor_write(sd, 0x30 + chn, 0x12);
	sensor_write(sd, 0xa0 + chn, 0x05);

	sensor_write(sd, 0xff, 0x01);
	sensor_write(sd, 0x84 + chn, 0x06);
	sensor_write(sd, 0x8c + chn, 0x86);
	sensor_read(sd, 0xed, &val);
	sensor_write(sd, 0xed, val & (~(0x01 << chn)));

	sensor_write(sd, 0xff, 0x05 + chn);
	sensor_write(sd, 0x20, 0x90);
	sensor_write(sd, 0x25, 0xcc);
	sensor_write(sd, 0x28, 0x80);
	sensor_write(sd, 0x2b, 0x78);
	sensor_write(sd, 0x47, 0x04);
	sensor_write(sd, 0x50, 0x84);
	sensor_write(sd, 0x58, 0x70);
	sensor_write(sd, 0x69, 0x00);
	sensor_write(sd, 0x7b, 0x00);
	sensor_write(sd, 0xb8, 0xb9);
}


void n5_set_chn_720h_ntsc(struct v4l2_subdev *sd, unsigned char chn)
{
	data_type val;

	sensor_write(sd, 0xff, 0x00);
	sensor_write(sd, 0x08 + chn, 0xa0);
	sensor_write(sd, 0x34 + chn, 0x00);
	sensor_write(sd, 0x81 + chn, 0x60);
	sensor_write(sd, 0x85 + chn, 0x00);
	...
	sensor_write(sd, 0x18 + chn, 0x08);
	sensor_write(sd, 0x58 + chn, 0x90);
	sensor_write(sd, 0x5c + chn, 0xbc);
	sensor_write(sd, 0x64 + chn, 0x81);
	sensor_write(sd, 0x89 + chn, 0x10);
	sensor_write(sd, chn + 0x8e, 0x2f);
	sensor_write(sd, 0x30 + chn, 0x12);
	sensor_write(sd, 0xa0 + chn, 0x05);
	sensor_write(sd, 0xff, 0x01);
	sensor_write(sd, 0x84 + chn, 0x06);
...
void n5_1080pX1(struct v4l2_subdev *sd)
{
	...
	sensor_dbg("set %s fps:%d\n", __func__, info->tpf.denominator);

	n5_regs_init_common(sd);

	if (info->tpf.denominator == 30)
		n5_set_chn_1080p_30(sd, 0);
	else if (info->tpf.denominator == 25)
...
void n5_cvbs_palX1(struct v4l2_subdev *sd)
{
	n5_regs_init_common(sd);
	n5_set_chn_720h_pal(sd, 0);
	n5_set_portmode(sd, 0, N5_OUTMODE_1MUX_SD, BT656);
}

void  n5_cvbs_ntscX1(struct v4l2_subdev *sd)
{
	n5_regs_init_common(sd);
	n5_set_chn_720h_ntsc(sd, 0);
	n5_set_portmode(sd, 0, N5_OUTMODE_1MUX_SD, BT656);
}
...
int n5_sensor_set_fmt(struct v4l2_subdev *sd,
					  struct v4l2_subdev_pad_config *cfg,
					  struct v4l2_subdev_format *fmt)
{
	struct sensor_info *info = to_state(sd);

	if (fmt->format.width == 720)	// NTSC/PAL
...
static int sensor_reg_init(struct sensor_info *info)
{
	...
	if (1920 == info->width && 1080 == info->height)
		n5_1080pX1(sd);
	else if (1280 == info->width && 720 == info->height)
		n5_720pX2(sd);
	else if (720 == info->width && 576 == info->height)
		n5_cvbs_palX1(sd);
	else if (720 == info->width && 480 == info->height)
		n5_cvbs_ntscX1(sd);
	return 0;
}
