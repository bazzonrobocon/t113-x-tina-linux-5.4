...
#ifdef CONFIG_SUNXI_TVD_AMP_SERVER

static int tvd_rv_start(struct tvd_dev *tvd)
{
	tvd_dbg("tvd_rv_start \n");
	//free_irq(tvd->irq, (void *) tvd);

	return 0;
}

static int tvd_rv_stop(struct tvd_dev *tvd)
{
	tvd_dbg("tvd_rv_stop \n");
	//ret = request_irq(tvd->irq, tvd_isr, IRQF_SHARED, tvd->v4l2_dev.name, tvd);

	return 0;
}

static struct tvd_amp_ctrl amp_ops = {
	.rv_start = tvd_rv_start,
	.rv_stop = tvd_rv_stop,
};
#endif
...
#ifdef CONFIG_SUNXI_TVD_AMP_SERVER
	if (dev && dev->rpmsg_tvd &&
			CONTROL_BY_RTOS == dev->rpmsg_tvd->control) {
		tvd_wrn("tvd is hold by risc-v\n");
		return -EBUSY;
	}


	if (amp_tvd_rpmsg_send(ARM_TVD_START)) {
		tvd_wrn("amp_tvd_rpmsg_send ARM_TVD_START fail\n");
		return -1;
	}
#endif
...
#ifdef CONFIG_SUNXI_TVD_AMP_SERVER
	if (amp_tvd_rpmsg_send(ARM_TVD_STOP)) {
		tvd_wrn("amp_tvd_rpmsg_send ARM_TVD_START fail\n");
		return -1;
	}
#endif
...
#ifdef CONFIG_SUNXI_TVD_AMP_SERVER
	tvd_dbg("register rpmsg \n");

	amp_tvd_init(&amp_ops, tvd[0]);
#endif
