 ...
 * drivers/input/sensor/sunxi_gpadc_battery.h
 *
 * Copyright (C) 2022 Allwinner.
 * zhengweilong <zhengweilong@allwinnertech.com>
 *
 * SUNXI GPADC BATTERY Controller Driver Header
 *
...
#ifndef __SUNXI_GPADC_BATTERY_H__
#define __SUNXI_GPADC_BATTERY_H__

#define GPADC_DEV_NAME ("sunxi-gpadc-battery")
...
#define GP_CH7_DATA_IRQ_EN (1 << 7) /* 0:disable, 1:enable */
...
#define GP_CH7_DATA (1 << 7) /* 0:no pending, 1:pending */
...
struct sunxi_config {
	...
	u32 resistance_before;
	u32 resistance_later;
};

/* Registers which needs to be saved and restored before and after sleeping */
static u32 sunxi_gpadc_battery_regs_offset[] = {
	GP_SR_REG,
	GP_CS_EN_REG,
	GP_CH0_CMP_DATA_REG,
	GP_CH1_CMP_DATA_REG,
...
struct sunxi_gpadc_battery {
	struct platform_device *pdev;
	struct input_dev *input_gpadc_battery[CHANNEL_MAX_NUM];
	...
	u32 wakeup_en;
	u32 channel_num;
	u32 gpadc_battery_sample_rate;
	struct delayed_work gpadc_battery_work;
	int bus_num;
	struct device *dev;
	struct clk *bus_clk;
	struct reset_control *reset;
	u32 regs_backup[ARRAY_SIZE(sunxi_gpadc_battery_regs_offset)];
};
