...
#define cdev_to_gpadcdev(i_cdev)	container_of(i_cdev, struct sunxi_gpadc, cdev)
...
{
	struct sunxi_gpadc *chip = cdev_to_gpadcdev(inode->i_cdev);

	filp->private_data = chip;
	return 0;
}

long channel_data_ioctl(struct file *filp, unsigned int cmd, unsigned long data)
{
	int ret;
	u32 data_reg, vin_m;
	struct sunxi_gpadc *chip = filp->private_data;

	switch (cmd) {
	case GPADC_CHANNEL0_DATA:
		data_reg = sunxi_gpadc_read_data(chip->reg_base, cmd);
		vin_m = ((VOL_RANGE / VOL_VALUE_MASK) * data_reg) / 1000;
		ret = copy_to_user((void *)data, &vin_m, sizeof(int));
		if (ret) {
			pr_err("copy_to_user fail\n");
			return -EINVAL;
		}
		break;
	case GPADC_CHANNEL1_DATA:
		data_reg = sunxi_gpadc_read_data(chip->reg_base, cmd);
		vin_m = ((VOL_RANGE / VOL_VALUE_MASK) * data_reg) / 1000;
		ret = copy_to_user((void *)data, &vin_m, sizeof(int));
		if (ret) {
		...
		break;
	default:
		pr_err("Please check if the channel parameters are correct, only 0 and 1 channels are supported!");
		return -EINVAL;
	}
...
int gpadc_release(struct inode *inode, struct file *filp)
{
	filp->private_data = NULL;
	return 0;
}

struct file_operations gpadc_fops = {
	.owner   = THIS_MODULE,
	.open = gpadc_open,
	.unlocked_ioctl = channel_data_ioctl,
...
#endif
	if (sunxi_gpadc->major) {
		sunxi_gpadc->devid = MKDEV(sunxi_gpadc->major, 0);
		ret = register_chrdev_region(sunxi_gpadc->devid, 1, "SUNXI_GPADC_DEV_NAME");
	} else {
		ret = alloc_chrdev_region(&sunxi_gpadc->devid, 0, 1, "SUNXI_GPADC_DEV_NAME");
		sunxi_gpadc->major = MAJOR(sunxi_gpadc->devid);
		sunxi_gpadc->minor = MINOR(sunxi_gpadc->devid);
	}
	sunxi_gpadc->cdev.owner = THIS_MODULE;
	cdev_init(&sunxi_gpadc->cdev, &gpadc_fops);
	cdev_add(&sunxi_gpadc->cdev, sunxi_gpadc->devid, 1);
	sunxi_gpadc->class = class_create(THIS_MODULE, "SUNXI_GPADC_DEV_NAME");
	sunxi_gpadc->device = device_create(sunxi_gpadc->class, NULL, sunxi_gpadc->devid, NULL, "SUNXI_GPADC_DEV");
...

{
	...
	device_destroy(sunxi_gpadc->class, sunxi_gpadc->devid);
	class_destroy(sunxi_gpadc->class);
	cdev_del(&sunxi_gpadc->cdev);
	unregister_chrdev_region(sunxi_gpadc->devid, 1);
	free_irq(sunxi_gpadc->irq_num, sunxi_gpadc);
	iounmap(sunxi_gpadc->reg_base);
	kfree(sunxi_gpadc);
