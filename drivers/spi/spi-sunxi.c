...
static void spi_set_rx_trig(u32 val, void __iomem *base_addr)
{
	...
	reg_val |= (val << SPI_FIFO_CTL_RX_TRIG_LEVEL) & SPI_FIFO_CTL_RX_LEVEL;
	writel(reg_val, base_addr + SPI_FIFO_CTL_REG);
...
static void spi_set_tx_trig(u32 val, void __iomem *base_addr)
{
	...
	reg_val |= (val << SPI_FIFO_CTL_TX_TRIG_LEVEL) & SPI_FIFO_CTL_TX_LEVEL;
	writel(reg_val, base_addr + SPI_FIFO_CTL_REG);
}
...
static int sunxi_spi_dma_tx_config(struct spi_device *spi, struct spi_transfer *t)
{
	...
	if (sspi->mode)
		spi_set_tx_trig(1, sspi->base_addr);
...
static int sunxi_spi_xfer(struct spi_device *spi, struct spi_transfer *t)
{
	...
	case QUAD_HALF_DUPLEX_TX:
	{
		...
		if (sunxi_spi_can_dma(spi->master, spi, t)) {
			...
			if (sspi->mode)
				spi_disable_irq(SPI_INTEN_RX_OVF, base_addr);
			if (!sspi->dbi_enabled)
				spi_start_xfer(base_addr);
	...
	*/
	...
	if (tcnt) {
		...
		if (!sunxi_spi_can_dma(spi->master, spi, tx)) {
			...
			ret = sunxi_spi_cpu_writel(spi, tx);
		} else {
	...
	if (rcnt) {
		if (!sunxi_spi_can_dma(spi->master, spi, rx)) {
			...
			ret = sunxi_spi_cpu_readl(spi, rx);
		} else {
...
static irqreturn_t sunxi_spi_handler(int irq, void *dev_id)
{
	...
	if (sspi->mode) {
		if ((enable & SPI_INTEN_RX_RDY) && (status & SPI_INT_STA_RX_RDY)) {
			...
			spi_disable_irq(SPI_INTEN_RX_RDY, base_addr);
			wake_up_process(sspi->task);
		} else if (status & SPI_INT_STA_TC) {
			dprintk(DEBUG_INFO, "[spi%d] SPI slave TC comes\n", sspi->master->bus_num);
			spi_disable_irq(SPI_INT_STA_TC | SPI_INT_STA_ERR, base_addr);

			/*wakup uplayer, by the sem */
			complete(&sspi->done);
		} else {
			dprintk(DEBUG_INFO, "[spi%d] slave unkown enable(%#x) status(%#x)\n",
			sspi->master->bus_num, enable, status);
		}
		spin_unlock_irqrestore(&sspi->lock, flags);
		return IRQ_HANDLED;
...
static int sunxi_spi_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(np, "spi_slave_mode", &sspi_mode);
	if (sspi_mode) {
		/* create spi slave */
	...
	if (sspi->mode) {
		...
		if (spi_register_master(master)) {
			SPI_ERR("[spi%d] cannot register SPI slave\n", sspi->master->bus_num);
			ret = -EBUSY;
			goto err6;
		}
	} else {
