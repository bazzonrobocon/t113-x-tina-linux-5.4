		   ...
		   struct cfg80211_scan_request *req)
{
	...
	if (priv->join_status != XRADIO_JOIN_STATUS_STA) {
		...
		for (i = 0; i < req->n_channels; i++) {
			...
			if (req->channels[i]->flags & (IEEE80211_CHAN_NO_IR | IEEE80211_CHAN_RADAR)) {
	...
	if (!hw_priv->scan.req || (hw_priv->scan.curr == hw_priv->scan.end)) {
		...
		for (it = hw_priv->scan.curr + 1, i = 1;
			...
			if ((*it)->band != first->band)
				break;
			if (((*it)->flags ^ first->flags) & (IEEE80211_CHAN_NO_IR | IEEE80211_CHAN_RADAR))
				break;
			if (!(first->flags & (IEEE80211_CHAN_NO_IR | IEEE80211_CHAN_RADAR)) &&
		...
		if (hw_priv->enable_advance_scan) {
			...
			scan.numOfProbeRequests =
			    (first->flags & (IEEE80211_CHAN_NO_IR | IEEE80211_CHAN_RADAR)) ? 0 : 4;
#ifdef CONFIG_XRADIO_TESTMODE
		}
#endif /* CONFIG_XRADIO_TESTMODE */
	...
	if (likely(atomic_xchg(&hw_priv->scan.in_progress, 0))) {
		...
		if (hw_priv->scan.status > 0) {
			...
			if (time_after(jiffies, (hw_priv->scan.scan_failed_timestamp +
				...
				&& !hw_priv->scan.scan_probe_resp) {
				printk(KERN_ERR "%s,scan timeout of fw exception cnt", __func__);
