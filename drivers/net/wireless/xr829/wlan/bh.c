...
#define SDIO_REG_CNT_INC(cnt) (cnt++)
#define TX_LIMIT_CNT_INC(cnt) (cnt++)
#else
#define SDIO_REG_CNT_INC(cnt)
#define TX_LIMIT_CNT_INC(cnt)
#endif
...
static int xradio_bh(void *arg)
{
	...
	for (;;) {
		...
		if (hw_priv->hw_bufs_used) {
			...
			++reg_read;
			SDIO_REG_CNT_INC(sdio_reg_cnt1);
			...
			if (ctrl_reg & HIF_CTRL_NEXT_LEN_MASK) {
				...
				SDIO_REG_CNT_INC(sdio_reg_cnt5);
			}
		}
#endif
		...
		if (!status) {
			...
			++reg_read;
#endif
			SDIO_REG_CNT_INC(sdio_reg_cnt2);
			if (ctrl_reg & HIF_CTRL_NEXT_LEN_MASK) {
...
data_proc:
		...
		if (rx) {
			...
			if (!(ctrl_reg & HIF_CTRL_NEXT_LEN_MASK)) {
				...
				++reg_read;
#endif
				SDIO_REG_CNT_INC(sdio_reg_cnt3);
				PERF_INFO_STAMP(&sdio_reg_time, &sdio_reg, 4);
			}
			PERF_INFO_STAMP(&bh_start_time, &bh_others, 0);
			...
			if (!read_len) {
				SDIO_REG_CNT_INC(sdio_reg_cnt6);
				rx = 0;
				goto tx;
			}
...
tx:
		...
		if (tx && tx_allowed) {
			...
			if (ret <= 0) {
				if (hw_priv->hw_bufs_used >= hw_priv->wsm_caps.numInpChBufs)
					TX_LIMIT_CNT_INC(tx_limit_cnt3);
#if BH_PROC_TX
				if (list_empty(&hw_priv->proc.bh_tx))
					TX_LIMIT_CNT_INC(tx_limit_cnt4);
#endif
				...
				if (tx_burst > 1 && tx_bursted < tx_burst_limit &&
					...
					if (tx_bursted >= tx_burst_limit)
						TX_LIMIT_CNT_INC(tx_limit_cnt5);
					if (tx_burst <= 1)
						TX_LIMIT_CNT_INC(tx_limit_cnt6);
#endif
				}
			}
		} else {
...
#if PERF_INFO_TEST
			if (!tx)
				TX_LIMIT_CNT_INC(tx_limit_cnt1);
			if (!tx_allowed)
				TX_LIMIT_CNT_INC(tx_limit_cnt2);
#endif
		}
		...
		if (!reg_read && hw_priv->hw_bufs_used >=
			...
			xradio_bh_read_ctrl_reg(hw_priv, &ctrl_reg);
			++reg_read;
			SDIO_REG_CNT_INC(sdio_reg_cnt4);
			PERF_INFO_STAMP(&sdio_reg_time, &sdio_reg, 4);
			if (ctrl_reg & HIF_CTRL_NEXT_LEN_MASK) {
				DBG_INT_ADD(fix_miss_cnt);
				rx = 1;
