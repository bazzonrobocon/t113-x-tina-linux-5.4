...
#include "../common/rpmsg_deinterlace.h"
#include "di_driver.h"
#include "di_fops.h"
#include "di_dev.h"
...
static int di_rv_start(void)
{
	...
	if (!amp_ops.rv_irq_state) {
		devm_free_irq(di_drvdata->pdev, di_drvdata->irq_no, (void *) di_drvdata);
...
static int di_rv_stop(void)
{
	...
	DI_DEBUG("di_rv_stop \n");
	if (amp_ops.rv_irq_state) {
		ret = devm_request_irq(di_drvdata->pdev, di_drvdata->irq_no,
				di_irq_handler, 0, dev_name(di_drvdata->pdev), di_drvdata);
