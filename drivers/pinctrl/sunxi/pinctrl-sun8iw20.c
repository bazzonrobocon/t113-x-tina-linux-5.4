...
static const unsigned sun8iw20_bank_base[] = {
	SUNXI_BANK_OFFSET('B', 'A'),
	SUNXI_BANK_OFFSET('C', 'A'),
	SUNXI_BANK_OFFSET('D', 'A'),
	SUNXI_BANK_OFFSET('E', 'A'),
...
static const struct sunxi_pinctrl_desc sun8iw20_pinctrl_data = {
	...
	.banks = ARRAY_SIZE(sun8iw20_bank_base),
	.bank_base = sun8iw20_bank_base,
...
{
	struct sunxi_pinctrl_desc const *desc = &sun8iw20_pinctrl_data;
	struct sunxi_pinctrl *pctl = dev_get_drvdata(dev);
	unsigned long flags;
	int idx, bank;
	...
	int power_mode_sel_reg = sunxi_pinctrl_hw_info[desc->hw_type].power_mode_sel_reg;
	int power_mode_ctrl_reg = sunxi_pinctrl_hw_info[desc->hw_type].power_mode_ctrl_reg;
	int pio_pow_ctrl_reg = sunxi_pinctrl_hw_info[desc->hw_type].pio_pow_ctrl_reg;

	raw_spin_lock_irqsave(&pctl->lock, flags);
	for (idx = 0; idx < desc->banks; idx++) {
		bank = desc->bank_base[idx];
		/* cfg/dat/drv/pull */
		memcpy_toio(pctl->membase + bank * bank_mem_size,
			    mem + bank * bank_mem_size,
			    bank_mem_size);
	}
	...
	for (idx = 0; idx < desc->irq_banks; idx++) {
		...
		memcpy_toio(pctl->membase + irq_cfg_reg + bank * irq_mem_size,
			   ...
			   0x10);
		/* irq debounce */
		memcpy_toio(pctl->membase + irq_debounce_reg + bank * irq_mem_size,
			    mem + irq_debounce_reg + bank * irq_mem_size,
			    0x4);
	}
	memcpy_toio(pctl->membase + power_mode_sel_reg, mem + power_mode_sel_reg, 0x4);
	memcpy_toio(pctl->membase + power_mode_ctrl_reg, mem + power_mode_ctrl_reg, 0x4);
	memcpy_toio(pctl->membase + pio_pow_ctrl_reg, mem + pio_pow_ctrl_reg, 0x4);
	raw_spin_unlock_irqrestore(&pctl->lock, flags);
